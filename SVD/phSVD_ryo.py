...
def rinput(drr='./'):
    ...
    if os.path.isfile(drr+'/DrivingTerms') and \
        ...
        for j in open(drr+'/Drive.inp'):
            ...
            if 'KICK =' in j: kick=float(j.split('=')[1])
...
def findpk(v,delta,x=None): # old --- remove
...
def peakInterpolate(dt,indx): # old ---- remove
...
def findTunes(data,q0,tol=0.1):#-- pk finder + quad interolate # old --- remove
...
def fTune(BX):#-- pk finder in window + quad interolate
