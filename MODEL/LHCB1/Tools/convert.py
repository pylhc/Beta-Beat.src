...
def getdata(plane):
   ...
   # print "the index of the bpms is ",names.tolist().index('BPM.29R4.B1')
    ...
    data = d[plane + 'PositionsConcentratedAndSorted'] # should be ok
    #data=d[plane + 'RawData']
    print len(data)
    #data = data.reshape(nbpms,nturns,nbunches) # wrong
...
a.treat_header()
sddsfile=open(options.PATH+"/"+os.path.basename(filename)+'.sdds.new','w')
nturns=''
nbunches=''
