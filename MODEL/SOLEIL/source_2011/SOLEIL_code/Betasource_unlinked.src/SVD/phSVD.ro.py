...
try:
    ...
    print "Using Numpy"
    from rhicdata25 import rhicdata; import pylab
...
def fTune(BX,BY):
    txx=[]; tyy=[]
    tx=abs(fftt(BX-average(BX,0), axis=0))
    ty=abs(fftt(BY-average(BY,0), axis=0))
...
if __name__ == "__main__":
    ...
    #indx,indy=findPairs(sx,sy)
    #for j in range(len(sx)):
    #    print sx[indx[j]], sy[indy[j]]
    #sys.exit()
