...
def runcorrection(nit,options,varlist):
    ...
    runcorrect=options.pyth+' '+options.loc+'/Correction/correct.iterative.py'
...
parser.add_option("-x", "--python",
                  ...
                  metavar="PY", default="pythonafs" , dest="pyth")
...
cpMAD='cp '+options.rpath+'/MODEL/'+options.ACCEL+'/job.iterative.correction.madx '+options.path
system(cpMAD)
...
print runMAD
# import knobs list
execfile(options.rpath+'/MODEL/'+options.ACCEL+'/AllLists_old.py')
