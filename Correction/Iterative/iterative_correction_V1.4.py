...
#from correctTESTFUNC2 import correctpy
...
def phasesubtract(prex,prey,nit,options,maccel,mpath):
    ...
    fmodel=options.rpath+'/MODEL/'+maccel+'/'+options.OPT+'/twiss.dat'
    ...
    for i in prex.NAME:
        ...
        try:
            ...
            0.0
    faftx.close()
    system('cp '+fpresentx+' '+mpath+'/getphasex.out')
    ...
    fafty.close()
    system('cp '+fpresenty+' '+mpath+'/getphasey.out')
...
def dispsubtract(predx,nit,options,maccel,mpath):
    ...
    for i in predx.NAME:
        try:
            ...
            0.0
    faftdx.close()
    system('cp '+fpresentDx+' '+mpath+'/getNDx.out')
...
def runcorrection(nit,options,varlist):
    ...
    if options.JustOneBeam=='0':
        optlist=[['-a ',options.ACCEL],['-t ',options.TECH],['-n ',options.ncorr],['-p ',options.path],['-c ',options.cut],['-e ',options.errorcut],['-m ',options.modelcut],['-r ',options.rpath],['-s ',options.MinStr],['-j ',options.JustOneBeam],['-v ',options.var],['-o ',options.OPT],['-w ',options.WGT]]
    if options.JustOneBeam=='1':
        optlist=[['-a ',options.ACCEL],['-b ',options.ACCEL2],['-t ',options.TECH],['-n ',options.ncorr],['-p ',options.path],['-q ',options.path2],['-c ',options.cut],['-e ',options.errorcut],['-m ',options.modelcut],['-r ',options.rpath],['-s ',options.MinStr],['-j ',options.JustOneBeam],['-v ',options.var],['-o ',options.OPT],['-w ',options.WGT]]
    runcorrect=options.pyth+' '+options.rpath+'/Correction/correct_twoB.py'
...
if j==1:
    ...
    file4nad.write('<'+options.rpath+'/MODEL/'+options.ACCEL2+'/'+options.OPT+'/'+'job.iterative.correction.mask > '+options.path2+'/job.iterative.correction.madx \n')
    file4nad.close()    
    os.system("chmod 777 "+str(filename))
    os.system(str(filename))
...
if j==1:
    command='cp '+options.path2+'/getphasex.out '+options.path2+'/getphasex.00.out'
    try:
        system(command)
    except:
...
if j==1:
    command='cp '+options.path2+'/getphasey.out '+options.path2+'/getphasey.00.out'
    try:
        system(command)
    except:
...
if j==1:
    command='cp '+options.path2+'/getNDx.out '+options.path2+'/getNDx.00.out'
    try:
...
if j==1:
    runMAD2=options.mad+'/madx < '+options.path2+'/job.iterative.correction.madx > out.mou'
# import knobs list
execfile(options.rpath+'/MODEL/'+options.ACCEL+'/'+options.OPT+'/AllLists.py')
if j==1:
    execfile(options.rpath+'/MODEL/'+options.ACCEL2+'/'+options.OPT+'/AllLists.py')
