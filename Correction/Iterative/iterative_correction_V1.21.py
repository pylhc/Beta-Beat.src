...
def runcorrection(nit,options,varlist):
    ...
    print "before I freeze"
    
    # Not clever implementation obliged by the fact that execfile("script.py") seems not to accept options for script.py.
    optlist=[['-a ',options.ACCEL],['-t ',options.TECH],['-n ',options.ncorr],['-p ',options.path],['-c ',options.cut],['-e ',options.errorcut],['-m ',options.modelcut],['-r ',options.rpath],['-s ',options.MinStr],['-j ',options.JustOneBeam],['-v ',options.var],['-o ',options.OPT]]
    runcorrect=options.pyth+' '+options.rpath+'/Correction/correct.py'
    #for i in range(0,len(optlist)):
    for i in range(0,1):
        runcorrect=runcorrect+' '+optlist[i][0]+str(optlist[i][1])
    print "triggereing correct.py  "+runcorrect
    #sys.exit()
    try:
        #sys.exit()
        #sys.path.append('/afs/cern.ch/eng/sl/lintrack/Beta-Beat.src///Numeric-23_p2.3/lib/python2.3/site-packages/Numeric:/afs/cern.ch/eng/sl/lintrack/Beta-Beat.src///Python_Classes4MAD/'
        system(runcorrect)
        #system()
       # sys.exit()

    except:
        print "not able to run correction for iterative correction"
...
#system("system, 'ln -nfs "+options.path+" filedes")
#print options.path
#sys.exit()
