## correction using beta instead of phase
...
parser.add_option("-e", "--errorcut",
                  help="Maximum error allowed for the beta measurement",
                  metavar="ERRORCUT", default=5.0 , dest="errorcut")
parser.add_option("-m", "--modelcut",
                  help="Maximum difference allowed between model and measured beta",
...
parser.add_option("-w", "--WhichBeta",
                  help="Beta from amplitude='amp' or phase='ph'(default)",
                  metavar="WHICHBETA", default="ph" , dest="whichbeta")
...
if options.whichbeta='ph':
    bx=twiss(options.path+'/getbetax.out')
    by=twiss(options.path+'/getbetay.out')
else:
    bx=twiss(options.path+'/getampbetax.out')
    by=twiss(options.path+'/getampbetay.out')
...
if options.ACCEL=="SPS":
	...
	#These kickers: MDH11605 MDHB61804
        ...
        intqx=26
        intqy=26
if options.ACCEL=="SLS":
    execfile(options.rpath+'/MODEL/SLS/AllLists.py')
    varslist=varSLS()
    intqx=20
    intqy=9
...
phasexlist=[]
phaseylist=[]
betaxlist=MakePairsBeta(bx, FullResponse['0'], modelcut=modelcut, errorcut=errorcut)
betaylist=MakePairsBeta(by, FullResponse['0'], modelcut=modelcut, errorcut=errorcut)
