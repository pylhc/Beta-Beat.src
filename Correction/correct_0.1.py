...
parser.add_option("-b", "--beta or phase",
                  help="Take beta from phase or amplitude",
                  metavar="beta", default="phase" , dest="beta")
...
if os.path.exists(options.path+'/getphasex_free.out'):
	print "Using _free!"
	x=twiss(options.path+'/getphasex_free.out')
...
if options.beta=="phase":
	...
	if os.path.exists(options.path+'/getbetax_free.out'):
		xbet=twiss(options.path+'/getbetax_free.out')
		ybet=twiss(options.path+'/getbetay_free.out')
	else:
	...
	if os.path.exists(options.path+'/getampbetax_free.out'):
		...
		xbet=twiss(options.path+'/getampbetax.out')
...
print "Will load full response from "+accelpath
...
print "weight=",wei
print "Beta at correct level ",len(betaxlist)
