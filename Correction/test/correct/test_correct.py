...
This module tests the output files of correct.py. First it runs the 'valid' version in the dir test/correct/
and then it runs the modified version in the dir Correction. After that it compares the content of the dirs
test.correct.data.output.valid and to_check.
'''
import unittest
import os
import sys
...
class TestCorrect(unittest.TestCase):
    ...
    PATH_TO_MODIFIED_SCRIPT = os.path.join(CURRENT_PATH, "..", "..", "correct.py")
    PATH_TO_VALID_SCRIPT = os.path.join(CURRENT_PATH, "valid_correct.py")
    ...
    def tearDown(self):
        if TestCorrect.__successful:
            self._delete_output()

    def testOutput(self):
        print "Start TestCorrect"
        for run_dir in self._run_dirs:
            ...
            if _SHORT_RUN:
                break
        TestCorrect.__successful = True
        print "End TestCorrect"
    ...
    def _check_if_valid_output_exist_and_set_attribute(self):
        if self._no_valid_output_exists():
            ...
            TestCorrect.__have_to_run_valid_file = True
    ...
    def _valid_output_exists(self):
        ...
        for dir_in_valid_output in all_dirs:
            is_valid = self._dir_has_valid_files(
                            os.path.join(TestCorrect.PATH_TO_VALID, dir_in_valid_output)
                            )
            if not is_valid:
                return False
        return True
    def _dir_has_valid_files(self, path_to_dir):
        """ Returns True if dir contains x, x.knob, x.madx and x.tfs.(x:=changeparameters). """
        has_all_files = True
        has_all_files = has_all_files and os.path.exists(os.path.join(path_to_dir, "changeparameters"))
        has_all_files = has_all_files and os.path.exists(os.path.join(path_to_dir, "changeparameters.knob"))
        has_all_files = has_all_files and os.path.exists(os.path.join(path_to_dir, "changeparameters.madx"))
        has_all_files = has_all_files and os.path.exists(os.path.join(path_to_dir, "changeparameters.tfs"))
    ...
    def _run_valid_file_if_necesaary(self, run_dir):
        ...
        if self.__have_to_run_valid_file:
            ...
            self._run_in_src_dir(TestCorrect.PATH_TO_VALID_SCRIPT, run_dir, os.path.join(TestCorrect.PATH_TO_VALID, run_dir))

    def _run_in_src_dir(self, path_to_script, run_dir_name, output_dst_path):
        """
        correct.py produces files which have the output dir as path in it. Thus two produced files in different
        ...
        for comparing.
        """
        output_path = os.path.join(TestCorrect.PATH_TO_INPUT, run_dir_name)
    ...
    def _run_modified_file(self, run_dir):
        ...
        self._run_in_src_dir(TestCorrect.PATH_TO_MODIFIED_SCRIPT, run_dir, os.path.join(TestCorrect.PATH_TO_TO_CHECK, run_dir))


    def _run_script(self, path_to_script, path_to_out_run_dir_root):
        ''' Runs given script with path_to_dir.
        path_to_dir has to contain getphase?.py and getbeta?.py or the equivalent free versions
    ...
    def _is_produced_output_file(self, file_path):
        return "changeparameters" in file_path
    ...
    def _prepare_and_get_arguments_list(self, path_to_out_run_dir_root):
        ...
        args_dict["--cut"] = "0.1"
        args_dict["--errorcut"] = "0.013,0.2"
        args_dict["--modelcut"] = "0.02,0.2"
        args_dict["--rpath"] = TestCorrect.BETABEAT_ROOT
        args_dict["--MinStr"] = "0.000001"
        args_dict["--weight"] = "1,1,0,0,1,10"
        args_dict["--JustOneBeam"] = "0" # 0 == true; 1 == false;
        args_dict["--ncorr"] = "5"
        args_dict["--tech"] = "SVD"
        args_dict["--opt"] = TestCorrect.PATH_TO_INPUT # We store fullresponse only once since it has around 20mb
        args_dict["--Variables"] = "MQTb1"
