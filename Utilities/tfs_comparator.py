...
def compare_tfs(tfs_file_1, tfs_file_2, max_relative_error=1e-9, check_globals=False, check_same_columns=False, ignore_names=[]):
    ...
    if not isinstance(tfs_file_1, twiss):
        try:
            ...
            print >> sys.stderr, "File", str(tfs_file_1), "unreadable"
            return False
    if not isinstance(tfs_file_2, twiss):
        try:
            tfs_file_2 = twiss(tfs_file_2)
        except Exception:
            print >> sys.stderr, "File", str(tfs_file_2), "unreadable"
            return False
    ...
    if check_same_columns and len(attr_list[0]) != len(attr_list[1]):
        print >> sys.stderr, "The files don't have the same number of columns or global parameters."
        return False
    ...
    for item_name in attr_list[longest_list]:
        if item_name not in ignore_names:
            if item_name not in attr_list[shortest_list] and check_same_columns:
                print >> sys.stderr, "The column", item_name, " is not present in both files."
                return False
                ...
                try:
                    ...
                    if type(next_value) is str:
                        raise TypeError
                    result = _check_column_values(item_name, next_value, getattr(tfs_files[longest_list], item_name),
                                                  max_relative_error)
                except TypeError:
                    if check_globals:
                        if not callable(next_value):
                            result = _check_single_value(item_name, next_value, \
                                                         getattr(tfs_files[longest_list], item_name), max_relative_error)
            if not result:
                break
    return result
...
def _check_column_values(column_name, column1, column2, max_relative_error):
    if len(column1) != len(column2):
        print >> sys.stderr, "The columns named", column_name, "doesn't have the same size."
        return False
    line = 1
    result = True
    for value1, value2 in zip(column1, column2):
        ...
        line += 1
    if not result:
        print >> sys.stderr, "Differences found in column", column_name, ". Aborting."
    return result
...
def _check_single_value(value_name, value1, value2, max_relative_error, line=-1):
    if type(value1) != type(value2):
        ...
        print >> sys.stderr, "Values of different type found on", value_name, line_string
        return False
    try:
        result = _compare_numeric_values(float(value1), float(value2), max_relative_error)
        if not result:
            line_string = "" if line == -1 else "(row " + str(line) + ")"
            print >> sys.stderr, "Difference found on", value_name, line_string + ":", str(value1), "!=", str(value2)
            return False
