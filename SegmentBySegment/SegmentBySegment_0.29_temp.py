...
#	sys.exit()
	...
	for bpm in bpms:
		...
		try:
			...
			print "ERROS: ",errx,erry,beta[0],beta[1]
			...
			if (betax>0) and (betay>0) and (errx<beta[2]) and (erry<beta[3]) and ((betax-errx)>0) and ((betay-erry)>0) and (errx<betax) and (erry<betay):
...
try:
	...
	filedatax=twiss(path+"/getbetay_free.out")	
betytwiss=filedatay
...
#			f1001m=coupling[1][bpm][8]
	...
	#		f1010b=abs(coupling[1][bpm][15])
		...
		erralfxp=abs(erralfmax.ALFX[erralfmax.indx[namename]]-erralfmin.ALFX[erralfmin.indx[namename]])
		erralfxb=abs(erralfmaxb.ALFX[erralfmaxb.indx[namename]]-erralfminb.ALFX[erralfminb.indx[namename]])
		erralfyp=abs(erralfmax.ALFY[erralfmax.indx[namename]]-erralfmin.ALFY[erralfmin.indx[namename]])
		erralfyb=abs(erralfmaxb.ALFY[erralfmaxb.indx[namename]]-erralfminb.ALFY[erralfminb.indx[namename]])

	
		fileresul.write("* METHOD  BETX ERRBETX  ALFX ERRALFX   BETY  ERRBETY   ALFY ERRALFY   DX   DPX   DY   DPY F1001  F1001STD F1010  F1010STD\n")
		fileresul.write("$  %s  %le  %le %le  %le  %le  %le %le  %le %le %le  %le  %le %le  %le  %le  %le\n")

		if coupleswitch==0:
			fileresul.write("normal "+str(hor[0])+" "+str(errbetxp)+" "+str(hor[1])+" "+str(erralfxp)+" "+str(ver[0])+" "+str(errbetyp)+" "+str(ver[1])+" "+str(erralfyp)+" "+str(dp[0])+" "+str(dp[1])+" "+str(dp[2])+" "+str(dp[3])+" "+str(0)+" "+str(0)+" "+str(0)+" "+str(0)+"\n")
			fileresul.write("back "+str(horb[0])+" "+str(errbetxb)+" "+str(horb[1])+" "+str(erralfxb)+" "+str(verb[0])+" "+str(errbetyb)+" "+str(verb[1])+" "+str(erralfyb)+" "+str(dpb[0])+" "+str(dpb[1])+" "+str(dpb[2])+" "+str(dpb[3])+" "+str(0)+" "+str(0)+" "+str(0)+" "+str(0)+"\n")
		else:
...
if flag==1:
	...
	resul.write('* NAME  S	    BETX  BETERRX   BETXMDL ALFX ERRALFX ALFXMDL  BETY  BETERRY  BETYMDL ALFY ERRALFY ALFYMDL  LEFTBPM   RIGHTBPM   VALID\n')
	resul.write('$ %s    %le    %le   %le    %le      %le   %le %le    %le   %le    %le      %le   %le   %le   %s  %s    %s\n')
	...
	for filee in files:
		...
		betx=[];errx=[];bety=[];erry=[];alfx=[];Aerrx=[];alfy=[];Aerry=[]
		...
		try:
			...
			alfx.append(one.ALFX[0])
			Aerrx.append(1/one.ERRALFX[0])
			alfy.append(one.ALFY[0])
			Aerry.append(1/one.ERRALFY[0])
			alfx.append(one.ALFX[1])
			Aerrx.append(1/one.ERRALFX[1])
			alfy.append(one.ALFY[1])
			Aerry.append(1/one.ERRALFY[1])			
			...
			alfxx=(1/(Aerrx[0]+Aerrx[1]))*(Aerrx[0]*alfx[0]+Aerrx[1]*alfx[1])
			alfyy=(1/(Aerry[0]+Aerry[1]))*(Aerry[0]*alfy[0]+Aerry[1]*alfy[1])
			errAx=sqrt((1/Aerrx[0])**2+(1/Aerrx[1])**2)/2
			errAy=sqrt((1/Aerry[0])**2+(1/Aerry[1])**2)/2
		...
		alfxmdl=twisstwiss.ALFX[twisstwiss.indx[name4colli]]
		alfymdl=twisstwiss.ALFY[twisstwiss.indx[name4colli]]
		S=twisstwiss.S[twisstwiss.indx[name4colli]]

		print >> resul,name4colli,S,betxx,errbetx,betxmdl,alfxx,errAx,alfxmdl,betyy,errbety,betymdl,alfyy,errAy,alfymdl,one.BPMSTART,one.BPMEND,tag

		######## IP
		resulip=open(savepath+"/summary_IP__"+options.accel+".tfs","w")

		print >> resulip,"name s betaxip errbetaxip betaxs errbetxs x ex betayip errbetayip betays errbetys y ey"
		print >> resulip,"%s %le %le  %le %le %le %le %le %le %le %le %le %le"

		# x
		betastarx=betxx/(1+alfxx**2)
		x=alfxx*betastarx #(sign flip)

		dx=((errAx/abs(alfxx))+(errbetx/abs(betxx)))*abs(x)
		dbx=sqrt(errbetx**2+(((-2*alfxx)/(1+alfxx**2)**2)*errAx)**2 )

		#y

		betastary=betyy/(1+alfyy**2)
		y=alfyy*betastary #(sign flip)

		dy=((errAy/abs(alfyy))+(errbety/abs(betyy)))*abs(y)
		dby=sqrt(errbety**2+(((-2*alfyy)/(1+alfyy**2)**2)*errAy)**2 )

		print >> resulip,name4colli,S,betxx,errbetx,betastarx,dbx,x,dx,betyy,errbety,betastary,dby,y,dy
