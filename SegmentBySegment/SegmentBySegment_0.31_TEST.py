...
#  !=>  SegementBySegment_0.31_TEST.py : - Output of the C-Matrix at given point.
...
def filterbpm(betaxx,betayy,commonbpms):
	...
	for bp in range(len(commonbpms)):
		...
		if (betax>0) and (betay>0) and (errx<betax) and (erry<betay) and (errx>0) and (erry>0):
			...
			if errx/betax>0.20:
				print "WARNING : relative horizontal errorbar on BPM ",bpm," is bigger then 20%"
			if erry/betay>0.20:
				print "WARNING : relative vertical errorbar on BPM ",bpm," is bigger then 20%"
...
def getTwiss(filee,element,cmatrix_switch):
    ...
    if cmatrix_switch=="1":
	    filedatax.Cmatrix()
	    cmatrix=filedatax.C[filedatax.indx[element]]
...
#	start="IP2"   #  compatible with repository
    ...
    file4nad.write('    -e \'s/%ELEMENT/\''+name+'\'/g\' \\\n')
    ...
    if "RHIC" in options.accel:
	   ...
	   file4nad.write('<'+cpath+'/SegmentBySegment/'+'/job.InterpolateBetas.0_1_RHIC.mask > '+path+'/t_'+str(name)+'.madx \n') 
	    ...
	    file4nad.write('<'+cpath+'/SegmentBySegment/'+'/job.InterpolateBetas.0_3.mask > '+path+'/t_'+str(name)+'.madx \n')
...
for namename in names:
	...
	if options.holyswitch=="0":
		...
		elementswitch=1
		print "Instrument has been choosen"
	...
	if (start[namename] not in goodbpms) or (end[namename] not in goodbpms):
		...
		endbpm=end[namename]
		print "The following bpms will be used as start and end point ",startbpm,endbpm
...
#	sys.exit()
	...
	if passs==1:
		...
		if coupleswitch==1:
			...
			try:
				...
				cpass=0
				print "startbpm ",startbpm," not found in measurement => values=0"
	...
	if elementswitch==0:
	##################
		[hor,ver,dp,cmatrix]=getTwiss(savepath+"/StartPoint_"+str(namename)+".twiss",startbpm,"0")
		startpos=hor[2]
		[hor,ver,dp,cmatrix]=getTwiss(savepath+"/twiss_"+str(namename)+".dat",startbpm,"0")
		betx=hor[0]
		bety=ver[0]
		ax=hor[1]
		ay=ver[1]
		...
		dy=dp[2]
		dpy=dp[3]
		[hor,ver,dp,cmatrix]=getTwiss(savepath+"/StartPoint_"+str(namename)+".twiss",endbpm,"0")
		endpos=hor[2]

		[hor,ver,dp,cmatrix]=getTwiss(savepath+"/twiss.b+.dat",startbpm,"0")
		bplusx=hor[0]
		bplusy=ver[0]
		[hor,ver,dp,cmatrix]=getTwiss(savepath+"/twiss.b-.dat",startbpm,"0")
		bminx=hor[0]
		...
		[hor,ver,dp,cmatrix]=getTwiss(savepath+"/twiss.a+.dat",startbpm,"0")
		aplusx=hor[1]
		aplusy=ver[1]
		[hor,ver,dp,cmatrix]=getTwiss(savepath+"/twiss.a-.dat",startbpm,"0")
		aminx=hor[1]
		...
		#m=options.twiss
		[hor,ver,dp,cmatrix]=getTwiss(twissfile,startbpm,"0")
		beta4plot=hor[2]
		...
		#CMATRIX
		fileresul=open(savepath+'/sbs_Cmatrix_'+namename+'.out','w')
		print >> fileresul,"$ NAME S C11 C12 C21 C22 DET"
		print >> fileresul,"* %s %le %le %le %le %le %le"
		for name in normal_pro.NAME:
			
			C=normal_pro.C[normal_pro.indx[name]]
			C11=C[0]
			C12=C[1]
			C21=C[2]
			C22=C[3]

			S=normal_pro.S[normal_pro.indx[name]]

			detC=C11*C22-C12*C21
			print >> fileresul,name,S,C11,C12,C21,C22,detC
		...
		fileresulc=open(savepath+'/couple_'+namename+'.tfs','w')
	
		[hor,ver,dp,cmatrix]=getTwiss(savepath+"/twiss_"+str(namename)+".dat",namename,"1")
		[horm,verm,dpm,cmatrixm]=getTwiss(savepath+"/StartPoint_"+str(namename)+".twiss",namename,"1")
		[horb,verb,dpb,cmatrixd]=getTwiss(savepath+"/twiss_"+namename+"_back_rev.dat",namename,"0")
		...
		fileresul.write('@ POSEND %le '+str(twisstwiss.S[twisstwiss.indx[endbpm]])+'\n')
		fileresul.write('@ BPMEND %s '+str(endbpm)+'\n')
		fileresul.write("@ INSTRUMENT  %s "+namename+"\n")
		fileresul.write("@ S %le  "+str(hor[2])+"\n")

		fileresulc.write('@ POSSTART %le '+str(twisstwiss.S[twisstwiss.indx[startbpm]])+'\n')
		fileresulc.write('@ BPMSTART %s '+str(startbpm)+'\n')
		fileresulc.write('@ POSELEMENT %le '+str(twisstwiss.S[twisstwiss.indx[namename]])+'\n')
		fileresulc.write('@ POSEND %le '+str(twisstwiss.S[twisstwiss.indx[endbpm]])+'\n')
		fileresulc.write('@ BPMEND %s '+str(endbpm)+'\n')
		fileresulc.write("@ INSTRUMENT  %s "+namename+"\n")
		fileresulc.write("@ S %le  "+str(hor[2])+"\n")		
		...
		print >> fileresulc, "* C11 C12 C21 C22 C11M C12M C21M C22M"
		print >> fileresulc, "$ %le  %le %le  %le  %le  %le %le  %le"

		print >> fileresulc,cmatrix[0],cmatrix[1],cmatrix[2],cmatrix[3],cmatrixm[0],cmatrixm[1],cmatrixm[2],cmatrixm[3]
