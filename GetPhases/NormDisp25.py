...
zdpp=0
wf=[]
for i in range(0,len(mydp)):
	wf.append(mydp[i]/sum(mydp))
	if mydp[i]!=0.0 or mydp[i]!=0 :
		nzdpp+=1
zdpp=len(x1)-nzdpp
wf=array(wf)

if zdpp==0 :
	print 'Error: No data for dp/p=0.'
	excit()
...
for i in range(0,len(bpmsx1)):
	...
	bns1=bpmsx1[i][0]
	for c in x0:
            nd=c.DX[c.indx[bn1]]/sqrt(c.BETX[c.indx[bn1]])
	a.append(nd)
a=array(a)
avex0=average(a)
#print avex0
