...
/* Constructing a matrix with all the data from the pick-ups for iformat i=2, RHIC*/ 
	 ...
	 printf("name of pick-up 0: %s pos: %f first turn:%e\n", bpmname[0], bpmpos[0], matrix[0][0]);
...
#pragma omp critical
	   ...
	   /*printf("%d %e %e %e %e\n", ii,tune[0],phase[0],amplitude[0],amplitude[2]);*/
    {
	   /*for(kk=0;kk<300;kk++){
	   ...
	   if (amplitude[0]>0 && label[i]==1 && ii==i){
	     fprintf(linfx,"\n\"%s\" %e %d %d %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e ", bpmname[ii], bpmpos[ii], ii, label[ii], tune[0], phase[0]/360., amplitude[0], noise1, maxmin, amplitude[2]/amplitude[0], phase[2]/360.,co, co2, amplitude[1]/amplitude[0], phase[1]/360., amplitude[12]/amplitude[0], phase[12]/360., amplitude[6]/amplitude[0], phase[6]/360.);
