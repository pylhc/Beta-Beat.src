...
def GetCoupling1(MADTwiss, ListOfZeroDPPX, ListOfZeroDPPY, Q1, Q2):
    ...
    try:
        #TODO: check if there is a global variable outputpath if GetLLM will be run from the GUI(vimaier)
...
def GetFreePhase_Eq(MADTwiss,Files,Qd,Q,psid_ac2bpmac,plane,bd,op):
    ...
    psi14mdl=(append(psimdl[3:],psimdl[:3] +Q)-psimdl)%1
    psi15mdl=(append(psimdl[4:],psimdl[:4] +Q)-psimdl)%1
    psi16mdl=(append(psimdl[5:],psimdl[:5] +Q)-psimdl)%1
    psi17mdl=(append(psimdl[6:],psimdl[:6] +Q)-psimdl)%1
    ...
    psi14all=zeros((len(bpm),len(Files)))
    psi15all=zeros((len(bpm),len(Files)))
    psi16all=zeros((len(bpm),len(Files)))
    psi17all=zeros((len(bpm),len(Files)))
    for i in range(len(Files)):
        if plane=='H': psid=bd*2*pi*array([Files[i].MUX[Files[i].indx[b[1]]] for b in bpm])  #-- bd flips B2 phase to B1 direction
        if plane=='V': psid=bd*2*pi*array([Files[i].MUY[Files[i].indx[b[1]]] for b in bpm])  #-- bd flips B2 phase to B1 direction
        for k in range(len(bpm)):
        ...
        psi14=(append(psi[3:],psi[:3]+2*pi*Q)-psi)/(2*pi)  #-- phase range back to [0,1)      ###ADD###
        psi15=(append(psi[4:],psi[:4]+2*pi*Q)-psi)/(2*pi)  #-- phase range back to [0,1)
        psi16=(append(psi[5:],psi[:5]+2*pi*Q)-psi)/(2*pi)  #-- phase range back to [0,1)
        psi17=(append(psi[6:],psi[:6]+2*pi*Q)-psi)/(2*pi)  #-- phase range back to [0,1)
        for k in range(len(bpm)):
    ...
    for k in range(len(bpm)):
        ...
        bn1 = upper(bpm[k%len(bpm)][1])
        bn2 = upper(bpm[(k+1)%len(bpm)][1])
        bn3 = upper(bpm[(k+2)%len(bpm)][1])
        bn4 = upper(bpm[(k+3)%len(bpm)][1])
        bn5 = upper(bpm[(k+4)%len(bpm)][1])
        bn6 = upper(bpm[(k+5)%len(bpm)][1])
        bn7 = upper(bpm[(k+6)%len(bpm)][1])
...
def main(outputpath,files_to_analyse,twiss_model_file,dict_file="0",accel="LHCB1",lhcphase="0",BPMU="um",COcut=4000,NBcpl=2,TBTana="SUSSIX",higher_order=1):
    ...
    if with_linx_for_zero_dppx and with_liny_for_zero_dppy:
        ...
        for twiss_f in ListOfZeroDPPX: 
            Q1temp.append(mean(twiss_f.TUNEX))
        for twiss_f in ListOfZeroDPPY: 
            Q2temp.append(mean(twiss_f.TUNEY))
        Q1temp = mean(Q1temp)
        Q2temp = mean(Q2temp)
    ...
    #TODO: the entries of following both lists will never be accessed. Delete them? (vimaier)
    betaxlist=[]
    betaylist=[]
    ...
    if with_linx_for_zero_dppx and with_liny_for_zero_dppy:
        ...
        if NBcpl == 1:
            ...
            for i in range(len(bpms)):
                ...
                #-- Output zero if the model does not have couping parameters
                    ...
                    list_row_entries = ['"'+bn1+'"',bns1,len(ListOfZeroDPPX),(sqrt(fwqw[bn1][0][0].real**2+fwqw[bn1][0][0].imag**2)),fwqw[bn1][0][1],fwqw[bn1][0][0].real,fwqw[bn1][0][0].imag, 0.0, 0.0]
                tfs_file.add_table_row(list_row_entries)
            ...
            for i in range(len(bpms)):
                ...
                #-- Output zero if the model does not have couping parameters
                    ...
                    list_row_entries = ['"'+bn1+'"',bns1,len(ListOfZeroDPPX), sqrt(fwqw[bn1][0][0].real**2+fwqw[bn1][0][0].imag**2),fwqw[bn1][0][1],fwqw[bn1][0][0].real,fwqw[bn1][0][0].imag,sqrt(fwqw[bn1][0][2].real**2+fwqw[bn1][0][2].imag**2),fwqw[bn1][0][3],fwqw[bn1][0][2].real,fwqw[bn1][0][2].imag, 0.0, 0.0, 0.0, 0.0 ]
                tfs_file.add_table_row(list_row_entries)
            ...
            if with_ac_calc:
                ...
                try:
                    ...
                    for i in range(len(bpmsf)):
                        ...
                        #-- Output zero if the model does not have couping parameters
                            ...
                            list_row_entries = ['"'+bn1+'"',bns1,len(ListOfZeroDPPX), sqrt(fwqwf[bn1][0][0].real**2+fwqwf[bn1][0][0].imag**2),fwqwf[bn1][0][1],fwqwf[bn1][0][0].real,fwqwf[bn1][0][0].imag,sqrt(fwqwf[bn1][0][2].real**2+fwqwf[bn1][0][2].imag**2),fwqwf[bn1][0][3],fwqwf[bn1][0][2].real,fwqwf[bn1][0][2].imag, 0.0, 0.0, 0.0, 0.0 ]
                        tfs_file.add_table_row(list_row_entries)
                except: 
                    pass
                ...
                for i in range(len(bpmsf2)):
                    ...
                    #-- Output zero if the model does not have couping parameters
                        ...
                        list_row_entries = ['"'+bn1+'"',bns1,len(ListOfZeroDPPX),sqrt(fwqwf2[bn1][0][0].real**2+fwqwf2[bn1][0][0].imag**2),fwqwf2[bn1][0][1],fwqwf2[bn1][0][0].real,fwqwf2[bn1][0][0].imag,sqrt(fwqwf2[bn1][0][2].real**2+fwqwf2[bn1][0][2].imag**2),fwqwf2[bn1][0][3],fwqwf2[bn1][0][2].real,fwqwf2[bn1][0][2].imag, 0.0, 0.0, 0.0, 0.0 ]
                    tfs_file.add_table_row(list_row_entries)
        ...
        fwqw['DPP'] = 0
        #TODO: couplelist is never readed again(vimaier)
        couplelist = [fwqw]
