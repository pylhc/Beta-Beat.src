...
#define MAXPICK 1100
#define MAXTURNS 50010 /* Critical parameter in sussixfordiveNoO.f, same parameter name !*/
#define MAXTURNS4 200040 /*Always four times  MAXTURNS*/
#define MAXRUNS 100
...
public:
    ...
    static void formatLinFile(std::string linFilePath, const int tunecount, const double tunesum, const double tune2sum,
            const int nattunecount, const double nattunesum, const double nattune2sum, int plane_index, double* phaseFromAverage);
...
void getPhaseFromAverageTune(double, double, int, int, int, double*, double*);
...
inline void callSussixCalcr(double*, double*, double*, int*);
...
#pragma omp parallel for
        ...
        #pragma omp critical
        {
            ...
            if (amplitude[0] > 0 && BPMs[horizontalBpmIndex].pickedUp == true && horizontalBpmIndex == i) {
                filesHandler.linxFile <<  std::scientific << '"' << BPMs[horizontalBpmIndex].bpmName << "\" " << BPMs[horizontalBpmIndex].bpmPos << ' ' <<
                                                                                        ...
                                                                                        amplitude[12] / amplitude[0] << ' ' << phase[12] / 360. << ' ' <<
                                                                                        amplitude[ 6] / amplitude[0] << ' ' << phase[ 6] / 360. << ' ' <<
                                                                                        amplitude[14] / amplitude[0] << ' ' << phase[14] / 360. << ' ' <<
                                                                                        amplitude[16] / amplitude[0] << ' ' << phase[16] / 360. << ' ' <<
                                                                                        amplitude[18] / amplitude[0] << ' ' << phase[18] / 360. << ' ' <<
                                                                                        amplitude[20] / amplitude[0] << ' ' << phase[20] / 360. << ' ' <<
                                                                                        amplitude[22] / amplitude[0] << ' ' << phase[22] / 360. << ' ' <<
                                                                                        amplitude[24] / amplitude[0] << ' ' << phase[24] / 360. << ' ' <<
                                                                                        amplitude[26] / amplitude[0] << ' ' << phase[26] / 360. << ' ' <<
                                                                                        amplitude[28] / amplitude[0] << ' ' << phase[28] / 360. << ' ' <<
                ...
                printf("false == %12f > 0\n", amplitude[0]);
                filesHandler.rejectedBpmsFileX <<  std::scientific << '"' << BPMs[horizontalBpmIndex].bpmName << "\" " << BPMs[horizontalBpmIndex].bpmPos << ' ' <<
                                                                                        horizontalBpmIndex << ' ' << BPMs[horizontalBpmIndex].pickedUp << ' ' << tune[0] << ' ' <<
                                                                                        phase[0] / 360. << ' ' << "%AVG_MU" << ' ' << amplitude[0] << ' ' << noise1 << ' ' << maxmin << ' ' << amplitude[2] / amplitude[0] << ' ' <<
            ...
            if (amplitude[3] > 0 && BPMs[verticalBpmIndex].pickedUp == true && verticalBpmIndex == i + MAXPICK/2) {
                filesHandler.linyFile <<  std::scientific << '"' << BPMs[verticalBpmIndex].bpmName << "\" " << BPMs[verticalBpmIndex].bpmPos << ' ' <<
                                                                                        ...
                                                                                        amplitude[19] / amplitude[3] << ' ' << phase[19] / 360. << ' ' <<
                                                                                        amplitude[21] / amplitude[3] << ' ' << phase[21] / 360. << ' ' <<
                ...
                printf("false == %12f > 0\n", amplitude[3]);
                filesHandler.rejectedBpmsFileY <<  std::scientific << '"' << BPMs[verticalBpmIndex].bpmName << "\" " << BPMs[verticalBpmIndex].bpmPos << ' ' <<
                                                                                        verticalBpmIndex << ' ' << BPMs[verticalBpmIndex].pickedUp << ' ' << tune[1] << ' ' <<
                                            phase[3] / 360. << ' ' << "%AVG_MU" << ' ' << amplitude[3] << ' ' << noise1 << ' ' << maxmin << ' ' << amplitude[5] / amplitude[3] << ' ' <<
    ...
    double averageTuneX = tuneCalcData.tuneSumX / tuneCalcData.tuneCountX;
    double averageTuneY = tuneCalcData.tuneSumY / tuneCalcData.tuneCountY;
    double phaseFromAvgX[maxOfHorBpmsAndVerBpms];
    double phaseFromAvgY[maxOfHorBpmsAndVerBpms];
    getPhaseFromAverageTune(averageTuneX, averageTuneY, maxOfHorBpmsAndVerBpms, lastBpmIndexHor, lastBpmIndexVer, phaseFromAvgX, phaseFromAvgY);

    /* Sort and move the "@..." lines to the top of the _linx/y files */
    IoHelper::formatLinFile(filesHandler.linxFilePath,
            tuneCalcData.tuneCountX, tuneCalcData.tuneSumX, tuneCalcData.tune2sumX, tuneCalcData.nattuneCountX, tuneCalcData.nattuneSumX, tuneCalcData.nattune2sumX, 1, phaseFromAvgX);
    IoHelper::formatLinFile(filesHandler.linyFilePath,
            tuneCalcData.tuneCountY, tuneCalcData.tuneSumY, tuneCalcData.tune2sumY, tuneCalcData.nattuneCountY, tuneCalcData.nattuneSumY, tuneCalcData.nattune2sumY, 2, phaseFromAvgY);
}

void getPhaseFromAverageTune(double averageTuneX, double averageTuneY, int maxOfHorBpmsAndVerBpms, int lastBpmIndexHor, int lastBpmIndexVer, double *phaseFromAvgX, double *phaseFromAvgY){
    double PI = std::acos(-1);
    double xPhase = -2 * PI * averageTuneX;
    double yPhase = -2 * PI * averageTuneY;
    std::complex<double> complexAvgTuneX (std::cos(xPhase), std::sin(xPhase));
    std::complex<double> complexAvgTuneY (std::cos(yPhase), std::sin(yPhase));
...
void IoHelper::formatLinFile(std::string linFilePath,
        const int tunecount, const double tunesum, const double tune2sum, const int nattunecount, const double nattunesum, const double nattune2sum, int plane_index, double* phaseFromAverageTune) {
    ...
    if (tunecount > 0) {
         ...
         */
        std::string avgMuMarker = "%AVG_MU";
        int vertical_bpm_offset = 0;
        if(plane_index > 1) vertical_bpm_offset = inpData.pickStart + MAXPICK / 2;
        while(!linFile.rdstate()){
            ...
            getline(linFile, temp_str);
            if(temp_str[0] == '"'){
                size_t start_pos = temp_str.find(avgMuMarker);
                if(start_pos != std::string::npos){
                    std::stringstream ss;
                    ss << std::scientific <<  phaseFromAverageTune[bIndex - vertical_bpm_offset];
                    temp_str.replace(start_pos, avgMuMarker.length(), ss.str());
                }
...
OutFilesHandler::OutFilesHandler(std::string &dataFilePath){
    ...
    rejectedBpmsFileX << "* NAME S    BINDEX SLABEL TUNEX MUX AVG_MUX AMPX NOISE PK2PK AMP01 PHASE01 CO   CORMS AMP_20 PHASE_20 AMP02 PHASE02 AMP_30 PHASE_30 AMP_1_1 PHASE_1_1 AMP2_2 PHASE2_2 AMP0_2 PHASE0_2 NATTUNEX NATAMPX\n";
    rejectedBpmsFileX << std::scientific << "$ %s  %le %le   %le   %le %le  %le %le %le  %le  %le  %le    %le %le  %le   %le     %le  %le    %le   %le     %le    %le      %le   %le     %le   %le     %le     %le\n";
    rejectedBpmsFileY.open((IoHelper::workingDirectoryPath+'/'+bpmFileName+"_rejectedBpms_y").c_str());
    rejectedBpmsFileY << "* NAME S    BINDEX SLABEL TUNEY MUY AVG_MUY AMPY NOISE PK2PK AMP10 PHASE10 CO   CORMS AMP_1_1 PHASE_1_1 AMP_20 PHASE_20 AMP1_1 PHASE1_1 AMP0_2 PHASE0_2 AMP0_3 PHASE0_3 AMP_11 PHASE_11 NATTUNEY NATAMPY\n";
    rejectedBpmsFileY << std::scientific << "$ %s  %le %le   %le   %le %le %le %le %le  %le  %le  %le    %le %le  %le    %le      %le   %le     %le   %le     %le   %le     %le   %le     %le   %le    %le       %le\n";
