...
C
     ...
     &nf,nline,nlst,nrc,nsus,nt1,nt2,ntot,ntotal,nturn,ntwin,ntwix
      double precision eps,etune,tunex,tuney,tunez, xy(4404), tunexy(2), 
     & amplitude(14), phase(14), ox(300), ax(300), oy(300), 
     &ay(300)
      parameter(mterm=300)
...
c      write(*,*) path(1:i+12), i
      open(10,file = path(1:i+12), form='formatted',
     &status='unknown')
      read(10,'(4(/))')
      read(10,'(A)') ch
      ch1=ch(9:80)//' / '
...
C.....SMEAR CALCULATION: OUTPUT IN THE UNIT IUSME
      if(isme.eq.1) then
C        write(6,*)'SMEAR CALCULATION: OUTPUT IN THE UNIT',iusme
...
C=======================================================================
      ...
      double precision duepi,freq,pi,tune,tunelasr,tunenewt,tuneabt,
     &tuneabt2,x,xp
...
C        close(30)
      ...
      do na=1,narm
        if(meth.eq.1) then
C          tune(na)=tunenewt(x,xp,maxn,zw)
          tune(na)=tuneabt2(x,xp,maxn,zw)
        elseif(meth.eq.2) then
C          tune(na)=tunelasr(x,xp,maxn,zw)
          tune(na)=tuneabt(x,xp,maxn,zw)
        endif
...
C
      ...
      DOUBLE PRECISION FUNCTION TUNEABTNICOLO(X,XP,MAXN,ZW)              
      PARAMETER(MAXITER=100000)
...
C.............................................................
      ...
      DO NFT=NPMIN,NPMAX
        IF (ABS(ZSING(NFT)).GT.FTMAX) THEN
          FTMAX=ABS(ZSING(NFT))
          NFTMAX=NFT
          PMAX=ATAN2(dimag(ZSING(NFT)), dreal(ZSING(NFT))) ! PHASE
        END IF
      ENDDO
C................................................INTERPOLATION
     ...
     .       ATAN2(CF3*DSIN(PI/NPOINT),CF2+CF3*DCOS(PI/NPOINT))
        DeltaNFTMAX=(CF3)/(CF3+CF2)
        ACORR=FTMAX*(2*pi*DeltaNFTMAX)/(NPOINT*sin(pi*DeltaNFTMAX))
        FCORR=NFTMAX+DeltaNFTMAX
        PCORR=PMAX-DeltaNFTMAX*pi
      ELSEIF (CF3.LE.CF1) THEN                   
        ASSK=DFLOAT(NFTMAX-1)+NPOINT/PI*
     .       ATAN2(CF2*DSIN(PI/NPOINT),CF1+CF2*DCOS(PI/NPOINT))
        DeltaNFTMAX=(-CF1)/(CF1+CF2)
        ACORR=FTMAX*(2*pi*DeltaNFTMAX)/(NPOINT*sin(pi*DeltaNFTMAX))
        PCORR=PMAX-DeltaNFTMAX*pi
        FCORR=NFTMAX+DeltaNFTMAX 
      ENDIF
C........................................FINAL INTERPOLATION
      TUNEAB=1D+0-(ASSK-1D+0)/DFLOAT(NPOINT)
      IF (DABS(TUNEAB).GT.0.5D0) 
     .                   TUNEABT=TUNEAB-DSIGN(1D0,TUNEAB)
C............................................................  
...
C........................................FINAL INTERPOLATION
      TUNEAB=1D+0-(ASSK-1D+0)/DFLOAT(NPOINT)
      TUNEAB=1D+0-( FCORR-1D+0)/DFLOAT(NPOINT)
...
C........................................FINAL INTERPOLATION
      ...
      TUNEAB2=1D+0-(ASSK-1D+0)/DFLOAT(NPOINT)
      IF (DABS(TUNEAB2).GT.0.5D0) 
     .                TUNEABT2=TUNEAB2-DSIGN(1D0,TUNEAB2)
C............................................................  
...
C=======================================================================
     ...
     &maxn,mterm,narm,nrc,nt1,nt2,nturn,nturn2
      double precision duepi,xy(4404),eps
      complex zsing
      parameter(mterm=300)
      parameter(maxn=100000)
      double precision s,sp,x,xp,y,yp
