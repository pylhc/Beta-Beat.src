...
class BadBpmFile(object):
    ...
    def __write_header(self, filehandle_badbpm):
        ...
        filehandle_badbpm.write("@  FILE %s " + self.path_to_sddsfile + "\n")
        filehandle_badbpm.write("*  Plane NAME   S     TurnData\n")
        filehandle_badbpm.write("$  %s    %s     %le   %le\n")
...
class Bpms(object):
    ...
    def remove_badbpm_and_get_badbpms(self, badbpm_indices, reason):
        ...
        and returns a list from the given bad BPMs"""
        badbpms = []
        goodbpm_indices = range(self.get_number_of_bpms())
        for index in badbpm_indices:
            badbpms.append(BadBpm(self.bpms_name_location_plane[index], self.bpm_data[index], reason))
            goodbpm_indices.remove(index)

        # do not forget to update the good BPM indices (tbach)
        self.bpm_data = [self.bpm_data[x] for x in goodbpm_indices]
        self.bpms_name_location_plane = [self.bpms_name_location_plane[x] for x in goodbpm_indices]
