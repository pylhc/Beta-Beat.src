...
def merge_data(working_directory, magnet1, circuit1, magnet2, circuit2, beam, ip, tunemeasprecision):
    ...
    if ip in ['ip1', 'ip5', 'ip8', 'IP1', 'IP5', 'IP8']:
        ...
        for magnet, circuit in zip([magnet1,magnet2],[circuit1, circuit2]):
            tdatax = metaclass.twiss(os.path.join(working_directory, circuit+'_tune_x_'+str(beam).lower()+'.tfs'))
            tdatay = metaclass.twiss(os.path.join(working_directory, circuit+'_tune_y_'+str(beam).lower()+'.tfs'))
